...
#def get_iter(file, iterator):
#	pass

#def applymapping_json(file, mappings, increment):
	#result = []
	#pass

def elem_yielder(file, iterator):
	if len(iterator) != 0:
		if "[*]" not in iterator[0] and iterator[0] != "$":
			yield from elem_yielder(file[iterator[0]], iterator[1:])
		elif "[*]" not in iterator[0] and iterator[0] == "$":
			yield from elem_yielder(file, iterator[1:])
		elif "[*]" in iterator[0] and "$" not in iterator[0]:
			file_array = file[iterator[0].replace("[*]","")]
			for array_elem in file_array:
				yield from elem_yielder(array_elem, iterator[1:])
		elif iterator[0] == "$[*]":
			for array_elem in file:
				yield from elem_yielder(array_elem, iterator[1:])
	else:
		yield file
...
def applymapping_json(file, mappings, increment):
	...
	for m in mappings:
		...
		iterator = str(m.iterator).split('.')

		if "$" not in iterator[0]:
			print("Error parsing iterator. Aborting...")
			exit(1)

		gen = elem_yielder(file, iterator)
		...
		for elem in gen:
			...
			if autoinc:
				span = autoinc[0]
				increment[0] += 1
				subj = '<' + template[:start] + elem[col] + template[end:span[0]] + str(increment[0]) + ">"
			else:
				subj = '<' + template[:start] + elem[col] + template[end:] + ">"
			...
			for predicate in predobjmap:
				...
				if maptype == 'Template':
					...
					if autoinc:
						span = autoinc[0]
						obj = '<' + objtemplate[:start] + elem[col] + objtemplate[end:span[0]] + str(increment[0]) + ">"
					else:
						obj = '<' + objtemplate[:start] + elem[col] + objtemplate[end:] + ">"
				...
				if maptype == 'ParentTriplesMap':
					...
					# assuming reference triples are in the same elem
					print(parenttriplemap.template)
					...
					if autoinc:
						span = autoinc[0]
						obj = '<' + parenttriplemap.template[:start] + elem[col] + parenttriplemap.template[
																				  end:span[0]] + str(increment[0]) + ">"
					else:
						obj = '<' + parenttriplemap.template[:start] + elem[col] + parenttriplemap.template[end:] + ">"
...
def applymapping_csv(row, mappings, increment):
	result = []
	for m in mappings:
		submap = m.subjectmap
		subjmap = m.subjectmap.smap
...
def transform(f, sources, file_format, sep=None):

	if file_format in ['csv','tsv']:
		pdf = pd.read_csv(f, sep=sep)
		increment = [0]
		ttls = pdf.apply(applymapping_csv, args=(sources, increment), axis=1)
	elif file_format == 'json':
		with open(str(f)[7:]) as file_obj:
			pdf = json.load(file_obj)
			increment = [0]
			ttls = applymapping_json(pdf, sources, increment)
	return ttls
...
def main(mappingfile):
	...
	for r in sourceres:
		...
		if sid in rmlsources:
			rmlsource = get_rml_source(r, rmlsources[sid])
		else:
			rmlsources[sid] = get_rml_source(r)
	...
	for f in filesubj:
		if '.csv' in f:
			...
			print('File not supported. Exiting...')
			exit(1)

		ttls = transform(f, filesubj[f], file_format, sep)
		if file_format in ["csv", "tsv"]:
...
sourcequery = '''
  ...
  '''
	...
	#mappingfile = "/home/guillermobet/Documents/Fraunhofer/ProjectIASIS/SimpleRDFizer/config/mappingone.ttl"
	#main(mappingfile)
